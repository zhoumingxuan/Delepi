...
function buildDeliverableFieldDescription(deliveryType: ExecutorDeliveryType): string {
  switch (deliveryType) {
    case '线索集合':
      return '  - deliverable_filename  string类型  必填，非空，固定为 `clues.json`;文件内容必须是 JSON 对象，且只包含 `clues` 字段；`clues` 必须为非空 string[]，每项必须是有事实依据的【线索集合】且禁止出现任何指代性表述。写入后必须用 Python 核验 JSON 可解析且顶层字段只有 `clues`。';
    case '权威结论':
      return '  - deliverable_filename  string类型  必填，非空，固定为 `conclusions.json`;文件内容必须是 JSON 对象，且只包含 `conclusions` 字段；`conclusions` 必须为非空 string[]，每项必须是依据事实产生的可被完整证明的【权威结论】且禁止出现任何指代性表述。写入后必须用 Python 核验 JSON 可解析且顶层字段只有 `conclusions`。';
    case '方案':
...
- 4.工具调用必须有实际执行价值；禁止做占位调用、空转调用或无意义重试。
- 5.每个工作方式执行完成只输出与此工作方式有关的内容，禁止输出其他无关内容。
- 6.绝对禁止出现任何【幻觉行为】，例如以下情况（仅举例，类似情况应有更多）：
...
- 7.仅当前任务要求运行或验证，且真实执行报错指向缺少依赖时，才允许安装缺失依赖。
- 8.工具报错时基于完整错误修正工具名、参数或执行方式；同一问题修复重试超过 5 次仍失败时，停止并返回失败 JSON。
- 9.写 Python 代码并执行必须调用 run_with_python。
\`\`\`
