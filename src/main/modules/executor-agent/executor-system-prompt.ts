...
- 关于问题【为什么需要沉淀经验？】的解答如下：
  - a.目的1：执行任务时在【某个类型子流程】上反复试错直到成功，但由于任务的隔离性，执行后续其他任务时仍然会在【相同类型子流程】上反复试错直到成功，因此需要沉淀经验形成工具【避免出现不必要的重复试错】。
  - b.目的2：执行任务时在【某个类型子流程】上编写冗长脚本且存在【可固化逻辑】,但由于任务的隔离性，执行后续其他任务时仍然会在【相同类型子流程】编写冗长脚本，因此需要沉淀经验形成工具【固化一些逻辑尽量避免出现冗长脚本】。
  - c.目的3：执行任务时在【某个类型子流程】上高频调用系统工具且存在【可固化逻辑】导致执行效率并不高，因此需要沉淀经验形成工具【针对相同逻辑在确保准确执行的情况下，尽可能提高执行效率】。
\`\`\`

# 经验工具库使用说明（** 内部规则，必须严格遵守 **）
\`\`\`
- 经验工具库主目录："${options.scriptsToolsDirText.trim()}"。
- 经验工具库包含的工具，只能经过script_tool 系统工具进行【查看协议】并且【调用】。
- 经验工具必须先【查看协议】确认目标工具的参数结构，再【调用】；禁止凭记忆猜测参数直接调用。
- 经验工具库主目录严格禁止存放任何【任务产物】；但允许经验工具库下每个工具做【日志记录】。
- 经验工具库只能存放通过分析【可沉淀经验】形成的【经验工具】。
\`\`\`

# 经验工具构建规范（** 内部规则，必须严格遵守 **）
\`\`\`
