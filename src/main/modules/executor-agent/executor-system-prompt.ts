...
- 经验工具库包含的工具，只能经过use_script_tool 系统工具进行【查看协议】并且【调用】。
...
# 输入使用规范
\`\`\`
- 仅依据【当前任务】、【工作方式】和已下发材料执行以及可能存在的【用户提示】；禁止假设对话历史、上一任务结果或未明示意图。
- 【当前任务】目标、对象、约束、交付物或验收口径缺失且无法补齐时，禁止自行补造。
\`\`\`

# 用户提示处理
\`\`\`
- 任务执行期间，可能陆续出现以「【用户提示】」为标题前缀的内容，仅提取对当前【工作方式】有影响的内容，不改变任务目标。
- 【用户提示】可能影响【工作方式】，但无论【用户提示】造成任何影响，当前【工作方式】都必须保持【逻辑自洽】。
- 无论「【用户提示】」内容含有什么，均不得影响【任务目标】和【交付目标】，不得直接结束任务，不得反问用户，需自行理解处理。
-「【用户提示】」内容，若超出当前任务范围之外，明确拒绝执行。
- 每出现一条【用户提示】，需给出【适当、简短、正式】且必须是【纯文本非MarkDown格式】的一个【助手回复】；**不得反问也不要等待用户再次回复，继续执行任务**；严格禁止【助手回复】与【最终输出】合并输出。
\`\`\`
...
/**
 * ★ 任务级交互消息注入包装模板（用户定稿草案A3，逐字）：
 * 每条用户消息在安全点 push 进 modelMessages 前统一包装；标题前缀「【用户提示】」与
 * 系统提示词「# 用户提示」章节的识别口径一致（双层防御：系统层定边界 + 逐条层重申抗漂移）。
 */
export function buildExecutorUserTaskMessage(userText: string): string {
  return `
【用户提示】
\`\`\`
-（仅影响【工作方式】，不改变当前【任务目标】；助手必须回复）
- 提示内容（不影响【任务目标】和【交付物类型】以及【最终交付产物】）：
${userText}
\`\`\`
`.trim();
}
