...
- 新增工具触发条件：反复试错直到最终成功的操作，沉淀为一个通用工具；不停模糊查找执行路径、多次重复操作的过程，简化成一个直接性路径，沉淀为一个通用工具；执行脚本过长且存在大面积的【可复用、可固化】逻辑，沉淀为一个通用工具。
- 修改工具触发条件：工具存在适配场景缺口，工具 BUG 已在实际运行中暴露，修改该工具。
- 拆分工具触发条件：工具适用场景较少且整体逻辑较为复杂或颗粒度较大时，可将当前工具拆分成多个【互不耦合】的工具。
- 删除工具触发条件：工具数量超限（上限 32 个，经验库经验此时已几乎饱和），删除创建时间最早且适用范围最低的工具；或对任务【执行效率】收益不大时，删除符合的工具；并且必须注意：尽量避免删除重要工具。
\`\`\`
`.trim()
    : '';
...
flowchart TD
  ...
  O -- 是 --> O1[尝试提取执行过程中【可沉淀经验】，并同时记录到摘要中]
  O -- 否 --> N1
  N -- 否 --> N1[更新或修正当前任务进度]
  N1 --> G
