...
3.必须严格的按照时间顺序**原样保留**以下信息：1.用户提供的所有事实信息。2.所有已确认的【事实、结论、线索】。3.全部或者最近10条用户输入的信息中所有特征（如：数据特征、视觉特征等）。4.全部或者最近10条用户表达的意图。
4.必须写入全部或者最近10轮对话摘要以及执行任务摘要信息；同时务必体现对话的用户意图和具体时序。
5.仅输出MarkDown文档且只能有以下【互相不耦合】的五个章节：1.事实和结论清单。2.线索清单。3.用户输入信息中特征清单（同时说明具体时序）4.用户历史意图（同时说明具体时序）5.历史对话概要（同时说明具体时序）；绝不输出MarkDown文档以外的其他内容。
`.trim();
}
